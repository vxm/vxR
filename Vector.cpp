...
v2s::v2s(const v2s &other) : m_x(other.m_x), m_y(other.m_y) {}
...
void v2s::setX(const scalar x) { m_x = x; }

void v2s::setY(const scalar y) { m_y = y; }

v2s v2s::get() const { return *this; }
...
scalar v2s::x() const { return m_x; }

scalar v2s::y() const { return m_y; }

scalar &v2s::operator[](const unsigned int idx) { return idx == 0 ? m_x : m_y; }
...
v2s v2s::asIntPosition() const { return {floor(m_x), floor(m_y)}; }
...
v2s v2s::inverted() const { return {-m_x, -m_y}; }
...
scalar v2s::angle(const v2s &other) const { return angle() - other.angle(); }

scalar v2s::angle() const { return atan2(m_y, m_x); }
...
v3s::v3s(scalar x, scalar y, scalar z) : m_x(x), m_y(y), m_z(z) {}
...
void v3s::setX(scalar x) { m_x = x; }

void v3s::setY(scalar y) { m_y = y; }

void v3s::setZ(scalar z) { m_z = z; }
...
v3s v3s::tiny() const
{
	return {m_x * tinyScalar, m_y * tinyScalar, m_z * tinyScalar};
}

v3s v3s::small() const
{
	return {m_x * smallScalar, m_y * smallScalar, m_z * smallScalar};
}

scalar v3s::sqrDistance(const v3s &in) const
{
	return (in.m_x - m_x) * (in.m_y - m_y) * (in.m_z - m_z);
}
...
scalar v3s::x() const { return m_x; }

scalar v3s::y() const { return m_y; }

scalar v3s::z() const { return m_z; }

v2s v3s::xy() const { return {m_x, m_y}; }

v2s v3s::xz() const { return {m_x, m_z}; }

v2s v3s::yz() const { return {m_y, m_z}; }
...
v3s v3s::cross(const v3s &v) const { return (*this) ^ v; }
...
v3s v3s::inverted() const { return v3s(-m_x, -m_y, -m_z); }
...
{
	return sqrt(fabs(m_x * m_x) + fabs(m_y * m_y) + fabs(m_z * m_z));
}
...
v3s v3s::floorVector() const { return {floor(m_x), floor(m_y), floor(m_z)}; }

v3s v3s::ceilVector() const { return {ceil(m_x), ceil(m_y), ceil(m_z)}; }
...
bool v3s::follows(const v3s &direction) const { return dot(direction) < 0.0; }
...
scalar v3s::angleXY() const { return atan2(m_y, m_x); }

scalar v3s::angleYZ() const { return atan2(m_z, m_y); }
...
v3s v3s::abs() const { return v3s(fabs(m_x), fabs(m_y), fabs(m_z)); }
...
{
	return v3s(m_x / (scalar)factor, m_y / (scalar)factor, m_z / (scalar)factor);
}
...
{
	return v3s(other.m_x / m_x, other.m_y / m_y, other.m_z / m_z);
}
...
v3s v3s::operator-(scalar factor) const
{
	return v3s(m_x - factor, m_y - factor, m_z - factor);
}

v3s v3s::operator-(const v3s &other) const
{
	return v3s(m_x - other.m_x, m_y - other.m_y, m_z - other.m_z);
}
...
v2s v3s::vectorXY() const { return (v2s(m_x, m_y)); }

v2s v3s::vectorYZ() const { return (v2s(m_x, m_y)); }

v2s v3s::vectorZX() const { return (v2s(m_x, m_y)); }

bool v3s::xPositive() const { return m_x >= 0.0; }

bool v3s::yPositive() const { return m_y >= 0.0; }

bool v3s::zPositive() const { return m_z >= 0.0; }
