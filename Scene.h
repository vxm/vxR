...
public:
  ...
  void setImageProperties(
	  const std::shared_ptr<vxCore::ImageProperties> &imageProperties);
  ///
  /// \brief defaultLight
  /// \return
  ///
  ...
  /// \param defaultShader
  ///
  void setShader(const vxShaderHandle &defaultShader);
  ///
  /// \brief camera
  /// \return
  ///
  ...
  /// \return
  ///
  ImageHandle createImage(const std::string &path, const scalar gain,
						  const scalar gamma);
  ///
  ...
  /// \return
  ///
  ImageHandle getImage(const vxNodeHandle &node);
