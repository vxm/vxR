...
void GeoGrid::setResolution(unsigned int x, unsigned int y, unsigned int z)
{
	m_rx = x;
	m_ry = y;
	m_rz = z;
...
unsigned long GeoGrid::index(unsigned int a, unsigned int b,
                             unsigned int c) const
{
	return ((m_ry * m_rx) * c) + (m_rx * b) + a;
}

unsigned long GeoGrid::numVoxels() const { return size(); }

unsigned int GeoGrid::rx() const { return m_rx; }

void GeoGrid::setRx(unsigned int rx) { m_rx = rx; }

unsigned int GeoGrid::ry() const { return m_ry; }

void GeoGrid::setRy(unsigned int ry) { m_ry = ry; }

unsigned int GeoGrid::rz() const { return m_rz; }

void GeoGrid::setRz(unsigned int rz) { m_rz = rz; }

unsigned long GeoGrid::lookupVoxel(const v3s &v, int &a, int &b, int &c) const
{
...
unsigned long GeoGrid::linearLookupVoxel(const v3s &v, int &a, int &b,
                                         int &c) const
{
...
void GeoGrid::locateAndRegister(const TriRef &tri, unsigned long triangleID)
{
	...
	if (idx1 == idx2 && idx2 == idx3)
	{
		if (indexIsValid(idx1))
		{
			...
			{
				m_members[idx1] = std::make_unique<SearchResult>(
		...
		for (auto y = bMin; y <= bMax; y++)
			for (auto z = cMin; z <= cMax; z++)
			{
				...
				if (indexIsValid(idx))
				{
					if (!m_members[idx])
					{
						m_members[idx] = std::make_unique<SearchResult>(
						    0ul, std::make_shared<std::vector<unsigned long>>());
					}
