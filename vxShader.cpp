...
vxColor vxLambert::getColor(vxCollision &collide)
	{
/*
...
*/
		double distanciaCentro = vxVector2d(collide.getU()-.5,collide.getV()-.5).length();
		...
		float intens = m_scene == NULL ? 0 : m_scene->getLight(collide);
		...
		{
			collide.setColor((1-distanciaCentro)*intens*collide.getColor().getR(), (1-distanciaCentro)*intens*collide.getColor().getG(), (1-distanciaCentro)*intens*collide.getColor().getB());
			...
			
		{
			...
			{
				if ( (collide.getU()<.01 || collide.getU()>.99) || (collide.getV()<.01 || collide.getV()>.99))
				{
