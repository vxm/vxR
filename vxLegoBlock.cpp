...
std::shared_ptr<vxLegoBlock> vxLegoBlock::at(const vxVector3d &pos, double size)
{
	set(pos,size);
	return shared_from_this();
}

void vxLegoBlock::set(const vxVector3d &pos, double size)
{
	if(!m_useDefault)
	{
...
int vxLegoBlock::throwRay(const vxRay &ray, vxCollision &collide) const
{
...
bool vxLegoBlock::hasCollision(const vxRay &ray) const
{
	const auto& instance = m_useDefault ? m_default : m_instance[std::this_thread::get_id()];
