...
// simple
const std::regex var_string{attributeNameEqSp + "(\".*?\")"s};
const std::regex var_int{attributeNameEqSp + "(\\d+)"s};
const std::regex var_float{attributeNameEqSp + floatReg};
const std::regex var_brakets{attributeNameEqSp + "(\\[.*?\\])"s};

// complex numbers
const std::regex var_float_float{attributeNameEqSp + floatReg + spaceReg +
								 floatReg};

const std::regex var_float_float_float{attributeNameEqSp + floatReg + spaceReg +
									   floatReg + spaceReg + floatReg};

const std::regex var_int_int{attributeNameEqSp + intReg + spaceReg + intReg};

const std::regex var_int_int_int{attributeNameEqSp + intReg + spaceReg +
								 intReg + spaceReg + intReg};
...
public:
	...
	Status parseNodeBody(std::ifstream &inFile, const vxNodeHandle &node);
