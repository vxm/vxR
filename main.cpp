...
int executeRenderProcess(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc && scenePath.empty(); i++)
	{
		if (sceneParam)
		{
			scenePath = argv[i];
	...
	for (const auto &renderSettings :
		 sceneParser->getNodesByType("vxRenderSettings"))
	{
